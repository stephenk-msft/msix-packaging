...
typedef std::unique_ptr<void, unique_hash_handle_deleter> unique_hash_handle;
    ...
    bool SHA256::ComputeHash(std::uint8_t* buffer, std::uint32_t cbBuffer, std::vector<uint8_t>& hash)
    {
        ...
        ThrowStatusIfFailed(BCryptGetProperty(
            ...
            0),                         // Flags
        "failed computing SHA256 hash");
        ThrowIf(resultLength != sizeof(hashLength), "failed computing SHA256 hash")
    ...
    std::string Base64::ComputeBase64(std::uint8_t *buffer, std::uint32_t cbBuffer)
    {
        ...
        if (!CryptBinaryToStringW(buffer, cbBuffer, encodingFlags, nullptr, &encodedHashSize))
        {
            std::cout << "base64 error:" << std::hex << GetLastError();
            throw;
        }
        result.resize(encodedHashSize);
        if (!CryptBinaryToStringW(buffer, cbBuffer, encodingFlags, const_cast<wchar_t*>(result.data()), &encodedHashSize))
        {
            std::cout << "base64 error 2 :" << std::hex << GetLastError();
            throw;
        }
