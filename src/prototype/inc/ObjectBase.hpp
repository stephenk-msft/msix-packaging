...
// COPIED FROM MSIX but improved to a vector of bytes :D
#pragma once
...
static void ExactValueValidation(T value, T spec) {
    ThrowIf(spec != value, "Incorrect value specified at field.");
}
...
{
    ThrowIf(spec1 != value && spec2 != value, "Incorrect value specified at field.");
}
...
public:
    ...
    std::vector<std::uint8_t> GetBytes() { return this->value; }
};
...
public:
    ...
    {
        f(Field<index>(), std::forward<Args>(args)...);
...
public:
    ...
    {
        size_t result = 0;
        this->for_each([](auto& field, size_t& result)
        {   result += field.Size();
        }, result);
        return result;
    }
    ...
    std::vector<std::uint8_t> GetBytes()
    {
        ...
        this->for_each([](auto& field, std::vector<std::uint8_t>& bytes)
        {
